...
def load_all_volumes(input_dir, pred_dir, gt_dir):  
    """Load MRI and prediction volumes; GT optional."""
    files = os.listdir(input_dir)
    # Include cases ending with '_T1.nii.gz' or '-T1.nii.gz'
    cases = sorted([
    ...
    for case in cases:
        # MRI: try both suffixes
        for suffix in ("_T1.nii.gz", "-T1.nii.gz"):
            path = os.path.join(input_dir, f"{case}{suffix}")
            if os.path.exists(path):
                mri_dict[case] = sitk.GetArrayFromImage(sitk.ReadImage(path))
                break
        else:
            raise FileNotFoundError(f"MRI file not found for case '{case}'")
        # Prediction: try both '_predict_seg' and '-predict_seg'
        ...
        # GT optional
        gt_file = os.path.join(gt_dir, f"{case}.nii.gz")
        gt_dict[case] = (
            sitk.GetArrayFromImage(sitk.ReadImage(gt_file))
            if os.path.exists(gt_file) else None
        )
...
# Unique slider key per case and plane to avoid out-of-range issues
slider_key = f"slice_{selected_case}_{plane}"
slice_idx = st.sidebar.slider(
    'Slice', 0, max_idx - 1, default_idx,
    key=slider_key
)
...
# Blend  
def blend(base, overlay, alpha): return (base*(1-alpha[...,None]) + overlay*alpha[...,None]).astype(np.uint8)
...
if gt_sl is None:
    ...
    for lbl,color in zip([1,2,3],[[255,0,0],[0,255,0],[0,0,255]]): overlay_gt[gt_sl==lbl] = color
...
for lbl,color in zip([1,2,3],[[255,0,0],[0,255,0],[0,0,255]]): overlay_pred[pred_sl==lbl] = color
alpha_pred = (pred_sl>0).astype(float)*0.5
combined_pred = blend(base_rgb, overlay_pred, alpha_pred)
# Pred vs GT
if gt_sl is None: combined_pred_gt = combined_pred
else: combined_pred_gt = blend(combined_pred, overlay_gt, alpha_gt)

# Plotly figure  
fig = make_subplots(rows=2,cols=2,subplot_titles=(title_gt,'Prediction','MRI','Pred vs GT'),horizontal_spacing=0.01,vertical_spacing=0.1)
...
fig.update_layout(dragmode='pan',autosize=False,width=1600,height=1100,margin=dict(l=0,r=0,t=80,b=0),showlegend=False)
...
if gt_sl is not None:
    ...
    with col1: st.subheader('Segmentation Metrics Summary') ; st.table(metrics)
    if st.session_state.analysis_md is None: st.session_state.analysis_md = analyze_metrics_with_gpt(metrics)
    st.markdown(st.session_state.analysis_md,unsafe_allow_html=True)
else: st.info('Ground truth missing; skipping metrics')
